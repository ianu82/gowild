...
const REMOTE_BINARY_ENV_VAR: &str = "GOWILD_REMOTE_BINARY";
const SSH_CONTROL_SOCKET_NAME: &str = "ctl";
pub(crate) const REATTACH_COMMAND_ENV_VAR: &str = "GOWILD_REATTACH_COMMAND";

pub(crate) const REMOTE_KEYBINDINGS_ENV_VAR: &str = "GOWILD_REMOTE_KEYBINDINGS";
...
pub(crate) fn run_remote(remote: RemoteLaunch) -> io::Result<()> {
    ...
    let program = std::env::args()
        .next()
        .unwrap_or_else(|| "gowild".to_string());
    ...
    let prepared_remote = prepare_remote_gowild(&remote_ssh, remote.live_handoff)?;
    ensure_remote_server_ready(
        &remote_ssh,
        &prepared_remote.remote_gowild,
...
impl RemoteGoWild {
    fn for_platform(platform: RemotePlatform) -> Self {
        let install_suffix = ".local/bin/gowild".to_string();
...
struct PreparedRemoteGoWild {
    remote_gowild: RemoteGoWild,
...
impl RemoteSsh {
    ...
    fn install_gowild(&self, remote_gowild: &RemoteGoWild, source_path: &Path) -> io::Result<()> {
        let output = self.sh_output(&remote_install_prepare_script(remote_gowild))?;
...
fn remote_install_prepare_script(remote_gowild: &RemoteGoWild) -> String {
    format!(
        r#"set -eu
...
"#,
        install_suffix = remote_gowild.install_suffix
    )
}
...
fn prepare_remote_gowild(
    ssh: &RemoteSsh,
    live_handoff_enabled: bool,
) -> io::Result<PreparedRemoteGoWild> {
    let platform = detect_remote_platform(ssh)?;
    let remote_gowild = RemoteGoWild::for_platform(platform);
    let override_binary = remote_binary_override_path()?;
    let remote_binary_candidates = remote_binary_candidates(ssh, &remote_gowild)?;
    ...
    if override_binary.is_none() {
        for candidate in &remote_binary_candidates {
            if remote_binary_matches(ssh, candidate).unwrap_or(false) {
                return Ok(PreparedRemoteGoWild {
                    remote_gowild: candidate.clone(),
                    installed_or_replaced: false,
                    stop_after_install_approved: false,
                });
            }
        }
        if remote_binary_matches(ssh, &remote_gowild)? {
            return Ok(PreparedRemoteGoWild {
                remote_gowild,
                installed_or_replaced: false,
                stop_after_install_approved: false,
    ...
    if let Some(status_probe_gowild) = remote_binary_candidates.first().or_else(|| {
        remote_binary_exists(ssh, &remote_gowild)
            .ok()
            .and_then(|exists| exists.then_some(&remote_gowild))
    }) {
    ...
    confirm_remote_install(
        ssh.target(),
        &remote_gowild,
        &install_source_description(&remote_gowild.platform, override_binary.as_deref()),
    )?;
    let source = resolve_install_source(&remote_gowild.platform, override_binary)?;
    let install_result = ssh.install_gowild(&remote_gowild, &source.path);
    source.cleanup();
    install_result?;

    if !remote_binary_matches(ssh, &remote_gowild)? {
        return Err(io::Error::other(format!(
            "installed remote gowild at {}, but it did not report version {}",
            remote_gowild.shell_path,
            current_version()
        )));
    }
    warn_if_remote_bin_not_on_path(ssh)?;

    Ok(PreparedRemoteGoWild {
        remote_gowild,
...
fn remote_binary_candidates(
    ssh: &RemoteSsh,
    remote_gowild: &RemoteGoWild,
) -> io::Result<Vec<RemoteGoWild>> {
    let mut candidates = Vec::new();

    if let Some(path_candidate) = remote_binary_on_path_any(ssh, remote_gowild)? {
    ...
    let output = ssh.sh_output(&known_remote_binary_candidate_script(
        &remote_gowild.platform,
    ))?;
    if !output.status.success() {
        return Err(command_failed("remote binary discovery failed", &output));
    }
    let stdout = String::from_utf8_lossy(&output.stdout);
    for candidate in remote_gowilds_from_path_discovery(remote_gowild, &stdout) {
...
fn push_if_new_remote_binary_candidate(
    candidates: &mut Vec<RemoteGoWild>,
    candidate: RemoteGoWild,
) {
    if !candidates
        .iter()
...
if [ -n "$home" ]; then
    emit "$home/.local/bin/gowild"
fi
"#,
    );
    if platform.os == "macos" {
        script.push_str(
            r#"    emit "/opt/homebrew/bin/gowild"
    emit "/usr/local/bin/gowild"
"#,
        );
    } else if platform.os == "linux" {
        script.push_str(
            r#"    emit "/home/linuxbrew/.linuxbrew/bin/gowild"
"#,
        );
    }
    script.push_str(
        r#"if [ -n "$home" ]; then
    emit "$home/.local/share/mise/installs/gowild/$version/bin/gowild"
    emit "$home/.local/share/mise/installs/gowild/$version/gowild"
    emit "$home/.local/share/mise/installs/github-ogulcancelik-gowild/$version/gowild"
    emit "$home/.nix-profile/bin/gowild"
fi
if [ -n "$user" ]; then
    emit "/etc/profiles/per-user/$user/bin/gowild"
fi
emit "/nix/var/nix/profiles/default/bin/gowild"
emit "/run/current-system/sw/bin/gowild"
"#,
    );
...
fn remote_binary_on_path_any(
    ssh: &RemoteSsh,
    remote_gowild: &RemoteGoWild,
) -> io::Result<Option<RemoteGoWild>> {
    let output = ssh.user_shell_output("command -v gowild")?;
    if output.status.success() {
        let stdout = String::from_utf8_lossy(&output.stdout);
        if let Some(candidate) = remote_gowild_from_path_discovery(remote_gowild, &stdout) {
    ...
    let output = ssh.sh_output("command -v gowild\n")?;
    if !output.status.success() {
        return Ok(None);
    }

    let stdout = String::from_utf8_lossy(&output.stdout);
    Ok(remote_gowild_from_path_discovery(remote_gowild, &stdout))
}

fn remote_gowilds_from_path_discovery(
    remote_gowild: &RemoteGoWild,
    stdout: &str,
) -> Vec<RemoteGoWild> {
    stdout
        .lines()
        .filter_map(|path| remote_gowild_from_path(remote_gowild, path))
        .collect()
}

fn remote_gowild_from_path_discovery(
    remote_gowild: &RemoteGoWild,
    stdout: &str,
) -> Option<RemoteGoWild> {
    stdout
        .lines()
        .find_map(|path| remote_gowild_from_path(remote_gowild, path))
}

fn remote_gowild_from_path(remote_gowild: &RemoteGoWild, path: &str) -> Option<RemoteGoWild> {
    let path = path.trim();
    if !path.starts_with('/') {
        return None;
    }
    ...
    Some(remote_gowild.clone().with_shell_path(shell_quote(path)))
}

fn is_mise_shim_path(path: &str) -> bool {
    path.ends_with("/mise/shims/gowild")
}

fn remote_binary_matches(ssh: &RemoteSsh, remote_gowild: &RemoteGoWild) -> io::Result<bool> {
    let command = format!(
    ...
    Ok(version == format!("gowild {}", current_version())
        && parse_client_status_json(status)
            .map(|status| status.protocol == CURRENT_PROTOCOL)
            .unwrap_or(false))
}

fn remote_binary_exists(ssh: &RemoteSsh, remote_gowild: &RemoteGoWild) -> io::Result<bool> {
    let command = format!("test -x {}", remote_gowild.shell_path);
...
fn install_source_description_for(
    ...
    if local_binary_can_seed_remote {
        "the current local gowild binary".to_string()
    } else {
        format!(
            "the {} {} asset for {}",
            current_version(),
...
fn ensure_remote_server_ready(
    ...
    let status = remote_server_status(ssh, remote_gowild)?;
    ...
    if live_handoff_enabled && live_handoff {
        match live_handoff_remote_server(ssh, remote_gowild) {
            Ok(()) => return Ok(()),
            Err(err) => {
    ...
    if stop_after_install_approved {
        stop_remote_server(ssh, remote_gowild)?;
        return Ok(());
    }
...
fn confirm_remote_install_with_running_server(
    ...
    let status = match remote_server_status(ssh, remote_gowild) {
        Ok(status) => status,
        Err(err) => {
            if !io::stdin().is_terminal() {
                return Err(io::Error::other(format!(
                    "could not inspect the running remote gowild server on {target} before installing: {err}; run from an interactive terminal to approve updating the remote binary"
                )));
            }
            eprintln!(
                "could not inspect the running remote gowild server on {target} before installing: {err}"
            );
            eprint!("continue installing the remote gowild binary? [y/N] ");
            io::stderr().flush()?;
            ...
            if answer != "y" && answer != "yes" {
                return Err(io::Error::new(
                    ...
                    "remote gowild install cancelled",
                ));
            }
            return Ok(false);
        }
    ...
    if plan == RemoteInstallRunningServerPlan::KeepRunning {
        if io::stdin().is_terminal() {
            eprintln!("remote gowild server on {target} is already compatible:");
            eprintln!("  server: v{}", version_label(version.as_deref()));
            eprintln!(
                "GoWild will install {} without stopping the running remote server.",
                current_version()
            );
        }
        return Ok(false);
    ...
    if !io::stdin().is_terminal() {
        match plan {
            ...
            RemoteInstallRunningServerPlan::StopRequired(_) => {
                return Err(io::Error::other(format!(
                    "remote gowild server on {target} is running v{}; run from an interactive terminal to approve stopping it for the update",
    ...
    if plan == RemoteInstallRunningServerPlan::LiveHandoff {
        eprintln!("remote gowild server on {target} is currently running:");
        eprintln!("  server: v{}", version_label(version.as_deref()));
        eprintln!(
            "GoWild will install {} and hand off live pane processes to the prepared server.",
            current_version()
        );
        return Ok(false);
    }
    ...
    eprintln!("  server: v{}", version_label(version.as_deref()));
    eprintln!(
        "To complete the remote update, GoWild must stop the running remote server after installing."
    );
...
fn remote_server_status(
    ...
    let command = format!("{} status server --json", remote_gowild.shell_path);
    let output = ssh.sh_output(&command)?;
    if !output.status.success() {
...
fn confirm_remote_server_stop(
    ...
    if !io::stdin().is_terminal() {
        if reason == RemoteServerRestartReason::ProtocolMismatch {
            return Err(io::Error::other(format!(
                "remote gowild server on {target} must stop before this client can attach; run from an interactive terminal to approve stopping it"
            )));
        }

        eprintln!(
            "remote gowild server on {target} is still running v{}; it will use {} after it restarts.",
    ...
    match reason {
        ...
        RemoteServerRestartReason::DaemonDetachMissing => {
            eprintln!(
                "the remote server was started by a gowild build that may not survive SSH connection loss. restart it so network drops disconnect only this client."
            );
        }
        RemoteServerRestartReason::BinaryUpdated => {
            eprintln!(
                "the remote gowild binary was installed or replaced. restart the remote server so it uses the prepared binary."
            );
        }
        RemoteServerRestartReason::VersionMismatch => {
            eprintln!(
                "the remote server is still running a different gowild version. restart it so it uses the prepared binary."
            );
        }
    }
    ...
    if reason == RemoteServerRestartReason::ProtocolMismatch {
        return Err(io::Error::new(
            io::ErrorKind::Interrupted,
            "remote gowild server stop cancelled",
        ));
    }
...
fn live_handoff_remote_server(ssh: &RemoteSsh, remote_gowild: &RemoteGoWild) -> io::Result<()> {
    let command = format!(
    ...
    eprintln!(
        "handed off the remote gowild server on {}; reconnecting to the prepared server.",
        ssh.target()
    );
    Ok(())
}

fn stop_remote_server(ssh: &RemoteSsh, remote_gowild: &RemoteGoWild) -> io::Result<()> {
    let command = format!("{} server stop", remote_gowild.shell_path);
    let output = ssh.sh_output(&command)?;
    if !output.status.success() {
        return Err(command_failed("remote server stop failed", &output));
    }

    wait_for_remote_server_shutdown(ssh, remote_gowild)?;
    eprintln!(
        "stopped the remote gowild server on {}; it will restart when the remote client bridge attaches.",
        ssh.target()
    );
    Ok(())
}
...
fn wait_for_remote_server_shutdown(
    ...
    loop {
        if remote_server_status(ssh, remote_gowild)? == RemoteServerStatus::NotRunning {
            return Ok(());
        }
        if Instant::now() >= deadline {
            return Err(io::Error::new(
                ...
                format!(
                    "shutdown was requested, but the old remote gowild server on {target} is still responding after {} seconds",
...
fn warn_if_remote_bin_not_on_path(ssh: &RemoteSsh) -> io::Result<()> {
    ...
    eprintln!(
        "gowild: installed remote binary to ~/.local/bin/gowild, but the remote shell does not resolve `gowild` to that path"
    );
    Ok(())
}
...
fn remote_shell_resolves_managed_install(stdout: &str) -> bool {
    stdout
        ...
        .next()
        .map(str::trim)
        .is_some_and(|path| path.ends_with("/.local/bin/gowild"))
}
...
fn preview_assets_for_build<'a>(
    ...
    let build = manifest.builds.get(build_id).ok_or_else(|| {
        io::Error::other(format!(
            "preview manifest no longer includes build {build_id}; run `gowild update` locally or set {REMOTE_BINARY_ENV_VAR}=target/release/gowild"
        ))
    })?;
...
fn remote_release_asset(asset_key: &str) -> io::Result<RemoteReleaseAsset> {
    if crate::build_info::is_preview() {
        let build_id = crate::build_info::build_id().ok_or_else(|| {
            io::Error::other("preview client has no build id; set GOWILD_REMOTE_BINARY or install GoWild on the remote manually")
        })?;
        ...
        if protocol != CURRENT_PROTOCOL {
            return Err(io::Error::other(format!(
                "preview manifest has build {build_id} protocol {protocol}, but this client needs protocol {CURRENT_PROTOCOL}; set {REMOTE_BINARY_ENV_VAR}=target/release/gowild or install a matching GoWild on the remote host manually"
            )));
        }
    ...
    let release = manifest.release_for_version(&current_version).ok_or_else(|| {
        io::Error::other(format!(
            "release manifest does not include gowild {current_version}; build gowild for {} or install it there manually",
            asset_key
        ))
    })?;
    if let Some(protocol) = release.protocol {
        if protocol != CURRENT_PROTOCOL {
            return Err(io::Error::other(format!(
                "release manifest has gowild {current_version} protocol {protocol}, but this client needs protocol {CURRENT_PROTOCOL}; set {REMOTE_BINARY_ENV_VAR}=target/release/gowild or install a matching gowild on the remote host manually"
            )));
        }
    }
    let asset = release.assets.get(asset_key).ok_or_else(|| {
        io::Error::other(format!(
            "no {asset_key} binary in the release manifest for gowild {current_version}"
        ))
    })?;
...
fn private_download_dir(asset_key: &str) -> io::Result<PathBuf> {
    ...
    Err(io::Error::new(
        ...
        "failed to create private gowild remote download directory",
    ))
}
...
fn confirm_remote_install(
    ...
    if !io::stdin().is_terminal() {
        return Err(io::Error::other(format!(
            "matching remote gowild {} is not installed at {}; run from an interactive terminal to approve installation",
            current_version(),
            remote_gowild.shell_path
        )));
    }
    ...
    eprintln!(
        "matching gowild {} is not installed on {target} for {}.",
        current_version(),
        remote_gowild.platform.asset_key()
    );
    eprint!(
        "Install {} to {}? [Y/n] ",
        source_description, remote_gowild.shell_path
    );
    io::stderr().flush()?;
    ...
    if answer == "n" || answer == "no" {
        return Err(io::Error::new(
            io::ErrorKind::Interrupted,
            "remote gowild installation cancelled",
        ));
    }
...
fn remote_bridge_command(remote_gowild: &RemoteGoWild, session_name: &str) -> String {
    let mut command = format!("exec {}", remote_gowild.shell_path);
                ...
                match listener.accept() {
                    Ok(stream) => {
                        ...
                        if let Err(err) = bridge_connection(
                            ...
                            eprintln!("gowild: remote bridge failed: {err}");
                        }
                    }
                    ...
                    Err(err) => {
                        eprintln!("gowild: remote bridge listener failed: {err}");
                        break;
                    }
                }
            }
...
fn bridge_connection(
    ...
    command
        .arg("-T")
        .arg(target)
        .arg(remote_bridge_command(remote_gowild, session_name))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::inherit());
...
fn run_client_process(
    ...
    let status = Command::new(exe)
        ...
        .env("GOWILD_RENDER_ENCODING", "terminal-ansi")
...
fn local_forward_socket_path(target: &str, session_name: &str) -> PathBuf {
    ...
    let readable_name = format!("gowild-remote-{pid}-{target_clean}-{session_clean}.sock");
    let target_prefix: String = target_clean.chars().take(8).collect();
    let hash = short_socket_hash(target, session_name);
    let short_name = format!("gowild-r-{pid}-{target_prefix}-{hash}.sock");
...
mod tests {
    ...
    fn bridge_socket_is_user_only() {
        ...
        let socket = std::env::temp_dir().join(format!(
            "gowild-bridge-permissions-test-{}.sock",
            std::process::id()
        ));
        let remote_gowild = RemoteGoWild::for_platform(RemotePlatform {
            os: "linux",
            arch: "x86_64",
        });
    ...
    fn accepted_bridge_stream_is_reset_to_blocking() {
        ...
        let socket = std::env::temp_dir().join(format!(
            "gowild-bridge-blocking-test-{}.sock",
            std::process::id()
        ));
        let _ = std::fs::remove_file(&socket);
    ...
    fn managed_ssh_config_includes_user_config_then_fallback() {
        ...
        // gowild's fallback transport settings are present...
        assert!(
        ...
        if let Some(home) = std::env::var_os("HOME") {
            ...
            if user_config.is_file() {
                ...
                assert!(
                    ...
                    "user config must be Included before gowild's fallback: {contents}"
                );
            }
        }
    ...
    fn remote_install_stream_command_avoids_shell_c_wrapper() {
        let command = remote_install_stream_command("/home/a b/.local/bin/gowild.tmp.123");

        assert_eq!(command, "tee '/home/a b/.local/bin/gowild.tmp.123'");
    }
    ...
    fn remote_install_prepare_and_commit_scripts_quote_paths() {
        ...
        let prepare = remote_install_prepare_script(&remote_gowild);

        assert!(prepare.contains("mkdir -p \"$dir\""));
        assert!(prepare.contains("printf '%s\\0%s\\0' \"$tmp\" \"$dest\""));
        assert_eq!(
            parse_remote_install_paths(b"/home/a b/gowild.tmp.42\0/home/a b/gowild\0").unwrap(),
            (
                "/home/a b/gowild.tmp.42".to_string(),
                "/home/a b/gowild".to_string()
            )
        );
        assert_eq!(
            parse_remote_install_paths(b"/home/a b\n/gowild.tmp.42\0/home/a b\n/gowild\0").unwrap(),
            (
                "/home/a b\n/gowild.tmp.42".to_string(),
                "/home/a b\n/gowild".to_string()
            )
        );
        assert_eq!(
            remote_install_commit_script("/home/a b/gowild.tmp.42", "/home/a b/gowild"),
            "set -eu\nchmod 755 '/home/a b/gowild.tmp.42'\nmv '/home/a b/gowild.tmp.42' '/home/a b/gowild'\n"
        );
    }
    ...
    fn extract_remote_args_removes_space_form() {
        ...
        assert_eq!(cleaned, vec!["gowild", "--help"]);
    ...
    fn extract_remote_args_removes_equals_form() {
        let args = vec!["gowild".into(), "--remote=user@host".into()];
        let (cleaned, remote) = extract_remote_args(&args).unwrap();
        assert_eq!(cleaned, vec!["gowild"]);
        let remote = remote.unwrap();
    ...
    fn extract_remote_args_accepts_explicit_handoff() {
        let args = vec!["gowild".into(), "--remote=dev".into(), "--handoff".into()];
    ...
    fn extract_remote_args_preserves_handoff_without_remote() {
        let args = vec!["gowild".into(), "update".into(), "--handoff".into()];
    ...
    fn extract_remote_args_rejects_remote_keybindings_without_remote() {
        let args = vec!["gowild".into(), "--remote-keybindings=server".into()];
    ...
    fn extract_remote_args_requires_value() {
        let args = vec!["gowild".into(), "--remote".into()];
        let err = extract_remote_args(&args).unwrap_err();
    ...
    fn reattach_command_includes_remote_and_session() {
        assert_eq!(
            reattach_command(
                "target/release/gowild",
                "user@host",
                "work",
                RemoteKeybindings::Local,
                false,
            ),
            "target/release/gowild --remote user@host --session work"
        );
        assert_eq!(
            reattach_command(
                "gowild",
        ...
        assert_eq!(
            ...
            "gowild --remote host --remote-keybindings server"
        );
        assert_eq!(
            reattach_command(
                "gowild",
    ...
    fn windows_reattach_command_uses_current_executable() {
        ...
        assert_eq!(
            reattach_command(
                r"C:\Program Files\GoWild\gowild.exe",
                "host'name",
                "work'name",
                RemoteKeybindings::Local,
                false,
    ...
    fn remote_bridge_command_uses_installed_binary() {
        ...
        assert_eq!(
            remote_bridge_command(&remote_gowild, crate::session::DEFAULT_SESSION_NAME),
            "exec \"$HOME/.local/bin/gowild\" remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_uses_path_binary() {
        ...
        let remote_gowild = remote_gowild_from_path_discovery(&remote_gowild, "/usr/bin/gowild\n")
            .expect("path binary");

        assert_eq!(
            remote_bridge_command(&remote_gowild, crate::session::DEFAULT_SESSION_NAME),
            "exec /usr/bin/gowild remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_quotes_discovered_binary() {
        ...
        let remote_gowild =
            remote_gowild_from_path_discovery(&remote_gowild, "/opt/gowild bin/gowild\n")
                .expect("path binary");

        assert_eq!(
            remote_bridge_command(&remote_gowild, crate::session::DEFAULT_SESSION_NAME),
            "exec '/opt/gowild bin/gowild' remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_uses_macos_path_binary() {
        ...
        let remote_gowild =
            remote_gowild_from_path_discovery(&remote_gowild, "/opt/homebrew/bin/gowild\n")
                .expect("path binary");

        assert_eq!(
            remote_bridge_command(&remote_gowild, crate::session::DEFAULT_SESSION_NAME),
            "exec /opt/homebrew/bin/gowild remote-client-bridge"
        );
        assert_eq!(remote_gowild.platform.asset_key(), "macos-aarch64");
    }
    ...
    fn remote_path_discovery_reads_multiple_absolute_paths() {
        ...
        let candidates = remote_gowilds_from_path_discovery(
            &remote_gowild,
            "/usr/bin/gowild\nbin/gowild\n /opt/gowild bin/gowild\n",
        );

        assert_eq!(candidates.len(), 2);
        assert_eq!(candidates[0].shell_path, "/usr/bin/gowild");
        assert_eq!(candidates[1].shell_path, "'/opt/gowild bin/gowild'");
    }
    ...
    fn remote_path_discovery_ignores_mise_shims() {
        ...
        let candidates = remote_gowilds_from_path_discovery(
            &remote_gowild,
            "/home/can/.local/share/mise/shims/gowild\n/home/can/.local/share/mise/installs/gowild/0.7.1/bin/gowild\n",
        );
    ...
    fn known_remote_binary_candidate_script_includes_mise_and_nix_paths() {
        ...
        assert!(script.contains("emit \"$home/.local/bin/gowild\""));
        assert!(!script.contains("mise/shims/gowild"));
        assert!(script.contains(&format!("version={}", shell_quote(&current_version()))));
        assert!(
            script.contains("emit \"$home/.local/share/mise/installs/gowild/$version/bin/gowild\"")
        );
        assert!(script.contains("emit \"$home/.local/share/mise/installs/gowild/$version/gowild\""));
        assert!(script.contains(
            "emit \"$home/.local/share/mise/installs/github-ogulcancelik-gowild/$version/gowild\""
        ));
        assert!(script.contains("emit \"$home/.nix-profile/bin/gowild\""));
        assert!(script.contains("emit \"/etc/profiles/per-user/$user/bin/gowild\""));
        assert!(script.contains("emit \"/run/current-system/sw/bin/gowild\""));
        assert!(script.contains("emit \"/home/linuxbrew/.linuxbrew/bin/gowild\""));
        assert!(!script.contains("emit \"/opt/homebrew/bin/gowild\""));
    }
    ...
    fn known_remote_binary_candidate_script_includes_macos_homebrew_paths() {
        ...
        assert!(script.contains("emit \"/opt/homebrew/bin/gowild\""));
        assert!(script.contains("emit \"/usr/local/bin/gowild\""));
        assert!(!script.contains("emit \"/home/linuxbrew/.linuxbrew/bin/gowild\""));
    }
    ...
    fn remote_path_discovery_quotes_single_quotes_in_discovered_binary() {
        ...
        let remote_gowild =
            remote_gowild_from_path_discovery(&remote_gowild, "/opt/gowild's/bin/gowild\n")
                .expect("path binary");

        assert_eq!(
            remote_bridge_command(&remote_gowild, crate::session::DEFAULT_SESSION_NAME),
            "exec '/opt/gowild'\\''s/bin/gowild' remote-client-bridge"
        );
    }
    ...
    fn remote_path_discovery_ignores_relative_paths() {
        ...
        let remote_gowild = remote_gowild_from_path_discovery(&remote_gowild, "bin/gowild\n");

        assert!(remote_gowild.is_none());
    }
    ...
    fn remote_path_discovery_ignores_empty_output() {
        ...
        let remote_gowild = remote_gowild_from_path_discovery(&remote_gowild, "\n");
    ...
    fn remote_shell_path_warning_accepts_managed_install() {
        assert!(remote_shell_resolves_managed_install(
            "/home/can/.local/bin/gowild\n"
        ));
        assert!(remote_shell_resolves_managed_install(
            "/Users/can/.local/bin/gowild\n"
        ));
        assert!(!remote_shell_resolves_managed_install(
            "/usr/local/bin/gowild\n"
        ));
        assert!(!remote_shell_resolves_managed_install(""));
    }
    ...
    fn parse_client_status_json_reads_protocol() {
        assert_eq!(
            parse_client_status_json(r#"{"version":"x","protocol":8,"binary":"/bin/gowild"}"#)
    ...
    fn install_source_description_uses_override_binary() {
        ...
        assert_eq!(
            install_source_description_for(
                &platform,
                Some(Path::new("/tmp/gowild-aarch64")),
                false
            ),
            "GOWILD_REMOTE_BINARY (/tmp/gowild-aarch64)"
        );
    }
    ...
    fn install_source_description_uses_local_binary_when_allowed() {
        ...
        assert_eq!(
            ...
            "the current local gowild binary"
        );
    }
    ...
    fn resolve_install_source_uses_override_binary_without_temporary_cleanup() {
        ...
        let source = resolve_install_source(&platform, Some(PathBuf::from("/tmp/gowild-aarch64")))
            .expect("override source");
        assert_eq!(source.path, PathBuf::from("/tmp/gowild-aarch64"));
    ...
    fn windows_local_forward_endpoint_uses_private_state_dir() {
        ...
        assert!(path
            .file_name()
            .is_some_and(|name| name.to_string_lossy().starts_with("gowild-r-")));
    }
    ...
    fn local_forward_socket_path_uses_readable_name_when_it_fits() {
        ...
        let filename = path
            ...
            .to_string();
        assert!(
            filename.starts_with("gowild-remote-"),
    ...
    fn install_source_cleanup_removes_temporary_directory() {
        let dir = std::env::temp_dir().join(format!(
            "gowild-install-source-cleanup-test-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&dir);
