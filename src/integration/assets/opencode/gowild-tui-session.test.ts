...
beforeEach(() => {
  ...
  process.env.GOWILD_SOCKET_PATH = "test.sock";
...
async function loadPlugin() {
  importCounter += 1;
  const module = await import(`./gowild-tui-session.js?test=${importCounter}`);
...
test("retries an initial selection while GoWild detects the process", async () => {
  const plugin = await loadPlugin();
  const tui = fakeApi();
  tui.addSession({ id: "session-a" });
  tui.select("session-a");
