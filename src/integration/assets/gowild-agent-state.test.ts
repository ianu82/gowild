...
const originalEnvironment = {
  GOWILD_ENV: process.env.GOWILD_ENV,
  GOWILD_OMP_IDLE_DEBOUNCE_MS: process.env.GOWILD_OMP_IDLE_DEBOUNCE_MS,
  GOWILD_PANE_ID: process.env.GOWILD_PANE_ID,
  GOWILD_SOCKET_PATH: process.env.GOWILD_SOCKET_PATH,
};
...
const integrations = [
  { name: "Pi", modulePath: "./pi/gowild-agent-state.ts" },
  { name: "Oh My Pi", modulePath: "./omp/gowild-agent-state.ts" },
] as const;

const socketPlugins = [
  {
    name: "OpenCode",
    modulePath: "./opencode/gowild-agent-state.js",
    sessionID: "opencode-session",
  },
  { name: "Kilo", modulePath: "./kilo/gowild-agent-state.js", sessionID: "kilo-session" },
] as const;
...
function configureIntegrationEnvironment(recordingSocketPath: string) {
  process.env.GOWILD_ENV = "1";
  process.env.GOWILD_SOCKET_PATH = recordingSocketPath;
  process.env.GOWILD_PANE_ID = "test:p1";
}
...
async function startRecordingServer(name: string): Promise<unknown[]> {
  const recordingSocketPath = join(tmpdir(), `gowild-${name}-${process.pid}.sock`);
...
for (const socketPlugin of socketPlugins) {
  test(`${socketPlugin.name} maps the Windows socket marker path to a named pipe endpoint`, async () => {
    const markerPath = `gowild-${socketPlugin.name.toLowerCase()}-${process.pid}.sock`;
    configureIntegrationEnvironment(markerPath);
    Object.defineProperty(process, "platform", { value: "win32" });
    const connectedEndpoint = captureConnectionEndpoint();

    const { GoWildAgentStatePlugin } = await importFresh(socketPlugin.modulePath);
    const plugin = await GoWildAgentStatePlugin();
    await plugin.event({
      event: {
...
test("OpenCode stays disabled without the GoWild socket environment", async () => {
  process.env.GOWILD_ENV = "1";
  process.env.GOWILD_PANE_ID = "test:p1";
  delete process.env.GOWILD_SOCKET_PATH;

  const { GoWildAgentStatePlugin } = await importFresh("./opencode/gowild-agent-state.js");

  expect(await GoWildAgentStatePlugin()).toEqual({});
});

for (const integration of integrations) {
  test(`${integration.name} maps the Windows socket marker path to a named pipe endpoint`, async () => {
    const markerPath = `gowild-${integration.name.toLowerCase().replaceAll(" ", "-")}-${process.pid}.sock`;
    configureIntegrationEnvironment(markerPath);
    Object.defineProperty(process, "platform", { value: "win32" });
    const connectedEndpoint = captureConnectionEndpoint();
...
test("OMP accepts POSIX and Windows session paths", async () => {
  const { isAbsoluteSessionPath } = await importFresh("./omp/gowild-agent-state.ts");
...
test("Pi reports idle only after the agent settles", async () => {
  ...
  const { handlers, pi } = createExtensionHarness();
  const { default: install } = await importFresh("./pi/gowild-agent-state.ts");
  install(pi);
...
test("Pi settlement preserves explicit blocked-state precedence", async () => {
  ...
  handlers.get("agent_start")?.({}, context);
  await waitFor(() => requestStates(requests).length === 2);
  eventHandlers.get("gowild:blocked")?.({ active: true, label: "approval" }, context);
  await waitFor(() => requestStates(requests).length === 3);
  ...
  eventHandlers.get("gowild:blocked")?.({ active: false }, context);
...
test("Pi waits for a replacement session report before publishing state", async () => {
  const recordingSocketPath = join(tmpdir(), `gowild-pi-session-order-${process.pid}.sock`);
  socketPath = recordingSocketPath;
  await rm(recordingSocketPath, { force: true });
...
test("Oh My Pi retries working before a queued idle state", async () => {
  ...
  process.env.GOWILD_OMP_IDLE_DEBOUNCE_MS = "0";
  const { handlers, pi } = createExtensionHarness();

  const { default: install } = await importFresh("./omp/gowild-agent-state.ts");
  install(pi);
