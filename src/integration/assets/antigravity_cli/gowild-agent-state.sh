...
[ "${GOWILD_ENV:-}" = "1" ] || emit_and_exit
[ -n "${GOWILD_SOCKET_PATH:-}" ] || emit_and_exit
[ -n "${GOWILD_PANE_ID:-}" ] || emit_and_exit
...
seq = time.time_ns()
params = {
    "pane_id": os.environ["GOWILD_PANE_ID"],
    "source": "gowild:antigravity_cli",
    "agent": "agy",
    "seq": seq,
...
request = json.dumps({
    "id": f"gowild:antigravity_cli:{seq}",
...
try:
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as client:
        ...
        client.connect(os.environ["GOWILD_SOCKET_PATH"])
