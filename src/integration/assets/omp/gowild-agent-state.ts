...
const GOWILD_ENV = process.env.GOWILD_ENV;
const socketPath = process.env.GOWILD_SOCKET_PATH;
const socketEndpoint =
  process.platform === "win32" && socketPath ? `\\\\.\\pipe\\${socketPath}` : socketPath;
const paneId = process.env.GOWILD_PANE_ID;
const source = "gowild:omp";

function enabled() {
  return GOWILD_ENV === "1" && !!socketPath && !!paneId;
}
...
const idleDebounceMs = parseDurationEnv("GOWILD_OMP_IDLE_DEBOUNCE_MS", 250);
const retryGraceMs = parseDurationEnv("GOWILD_OMP_RETRY_GRACE_MS", 2500);
...
export default function (pi) {
  ...
  pi.events.on("gowild:blocked", (data) => {
    if (!rootSession) {
      return;
    }
    if (!data?.active) {
