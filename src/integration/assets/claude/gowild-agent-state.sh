...
action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/gowild-claude-hook.XXXXXX")" || exit 0
...
GOWILD_ACTION="$action" GOWILD_HOOK_INPUT_FILE="$hook_input_file" python3 - <<'PY'
import json
import os
import random
import socket
...
source = "gowild:claude"
action = os.environ.get("GOWILD_ACTION", "")
pane_id = os.environ.get("GOWILD_PANE_ID")
socket_path = os.environ.get("GOWILD_SOCKET_PATH")
hook_input_file = os.environ.get("GOWILD_HOOK_INPUT_FILE")
...
if hook_event_name == "SubagentStop":
    # SubagentStop is a completion event. Older GoWild integrations mapped it
