...
pub(crate) fn remove_legacy_pi_extension_from_omp_dir(dir: &Path) -> io::Result<bool> {
    ...
    if content.contains("GOWILD_INTEGRATION_ID=pi") {
...
pub(crate) fn install_antigravity_cli() -> io::Result<AntigravityCliInstallPaths> {
    ...
    // The GoWild block is GoWild-owned, so rewrite it wholesale and leave every
...
/// Builds the GoWild-owned `hooks.json` block for Antigravity CLI.
///
/// Every event GoWild registers takes a flat handler list; the `matcher`/`hooks`
/// group is only valid for the tool events, which GoWild does not use.
...
/// The complete GoWild-owned Grok hook config. Installation and status share
...
pub(crate) fn install_grok() -> io::Result<GrokInstallPaths> {
    ...
    // Grok merges every `~/.grok/hooks/*.json`, so gowild owns a dedicated
...
pub(crate) fn uninstall_grok() -> io::Result<GrokUninstallResult> {
    ...
    // gowild owns both files outright, so removal is a straight delete.
