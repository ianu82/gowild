const API_SCHEMA_JSON: &str = include_str!("../../docs/next/api/gowild-api.schema.json");
...
fn api_snapshot(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: gowild api snapshot");
        return Ok(2);
    }
...
fn schema_summary_text() -> std::io::Result<String> {
    ...
    Ok(format!(
        "GoWild API schema\nprotocol: {}\nschema_version: {}\nschemas: {}\n\nUse `gowild api schema --json` to print the full schema.\nUse `gowild api schema --output PATH` to write it to a file.\n",
        protocol,
        schema_version,
        schemas.join(", ")
    ))
...
fn print_api_help() {
    eprintln!("gowild api commands:");
    eprintln!("  gowild api snapshot");
    eprintln!("  gowild api schema [--json | --output PATH]");
}

fn print_api_schema_help() {
    eprintln!("usage: gowild api schema [--json | --output PATH]");
}
...
mod tests {
    ...
    fn schema_summary_text_stays_human_sized() {
        ...
        assert!(text.contains("GoWild API schema"));
        assert!(text.contains("Use `gowild api schema --json`"));
        assert!(text.len() < 400);
    }
}
