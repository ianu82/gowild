...
fn plugin_link(args: &[String]) -> std::io::Result<i32> {
    let Some(path) = args.first() else {
        eprintln!("usage: gowild plugin link <path> [--disabled]");
        return Ok(2);
    };
...
fn plugin_config_dir_command(args: &[String]) -> std::io::Result<i32> {
    let Some(plugin_id) = args.first() else {
        eprintln!("usage: gowild plugin config-dir <plugin_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gowild plugin config-dir <plugin_id>");
...
fn plugin_unlink(args: &[String]) -> std::io::Result<i32> {
    let Some(plugin_id) = args.first() else {
        eprintln!("usage: gowild plugin unlink <plugin_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gowild plugin unlink <plugin_id>");
...
fn plugin_install(args: &[String]) -> std::io::Result<i32> {
    let Some(source_arg) = args.first() else {
        eprintln!("usage: gowild plugin install <owner>/<repo>[/subdir...] [--ref REF] [--yes]");
        return Ok(2);
    };
...
fn plugin_uninstall(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: gowild plugin uninstall <plugin_id|owner/repo[/subdir...]>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gowild plugin uninstall <plugin_id|owner/repo[/subdir...]>");
...
fn plugin_action_invoke(args: &[String]) -> std::io::Result<i32> {
    let Some(action_id) = args.first() else {
        eprintln!("usage: gowild plugin action invoke <action_id> [--plugin ID]");
        return Ok(2);
    };
    let mut plugin_id = None;
    let mut index = 1;
...
fn plugin_pane_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(pane_id) = args.first() else {
        eprintln!("usage: gowild plugin pane focus <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gowild plugin pane focus <pane_id>");
...
fn plugin_pane_close(args: &[String]) -> std::io::Result<i32> {
    let Some(pane_id) = args.first() else {
        eprintln!("usage: gowild plugin pane close <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gowild plugin pane close <pane_id>");
...
impl GithubPluginSource {
    fn parse(value: &str) -> Result<Self, String> {
        ...
        if parts.len() < 2 {
            return Err("usage: gowild plugin install <owner>/<repo>[/subdir...]".into());
        }
        let owner = parts[0];
        let repo = parts[1];
...
fn verify_plugin_link_source_response(
        ...
        || plugin.source.managed_path != expected.managed_path
    {
        return Err(std::io::Error::other(
            "running GoWild server did not persist GitHub plugin source metadata",
        ));
    }
    Ok(())
}
...
fn ensure_manifest_unchanged_after_build(
    ...
    Err(io::Error::other(
        "plugin build changed gowild-plugin.toml after install preview; aborting install",
    ))
}
...
fn run_plugin_build_command(
    ...
    child
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    scrub_gowild_runtime_env(&mut child);
...
fn scrub_gowild_runtime_env(command: &mut Command) {
    for key in [
    ...
    for (key, _) in std::env::vars_os() {
        if key.to_string_lossy().starts_with("GOWILD_PLUGIN_") {
            command.env_remove(key);
        }
    }
}
...
fn plugin_checkout_lifecycle_error(operation: &str, path: &Path, err: io::Error) -> io::Error {
    if cfg!(windows) && err.kind() == io::ErrorKind::PermissionDenied {
        return io::Error::new(
            err.kind(),
            format!(
                "failed to {operation} managed plugin checkout at {}; close any GoWild plugin panes or plugin commands using that checkout, then retry: {err}",
                path.display()
            ),
        );
    }
...
fn print_plugin_help() {
    eprintln!("gowild plugin commands:");
    eprintln!("  gowild plugin install <owner>/<repo>[/subdir...] [--ref REF] [--yes]");
    eprintln!("  gowild plugin uninstall <plugin_id|owner/repo[/subdir...]>");
    eprintln!("  gowild plugin link <path> [--disabled]");
    eprintln!("  gowild plugin list [--plugin ID] [--json]");
    eprintln!("  gowild plugin config-dir <plugin_id>");
    eprintln!("  gowild plugin unlink <plugin_id>");
    eprintln!("  gowild plugin enable <plugin_id>");
    eprintln!("  gowild plugin disable <plugin_id>");
    eprintln!("  gowild plugin action <list|invoke>");
    eprintln!("  gowild plugin log list [--plugin ID] [--limit N]");
    eprintln!("  gowild plugin pane <open|focus|close>");
}

fn print_plugin_action_help() {
    eprintln!("gowild plugin action commands:");
    eprintln!("  gowild plugin action list [--plugin ID]");
    eprintln!("  gowild plugin action invoke <action_id> [--plugin ID]");
}

fn print_plugin_pane_help() {
    eprintln!("gowild plugin pane commands:");
    eprintln!("  gowild plugin pane open --plugin ID --entrypoint ID [--placement overlay|popup|split|tab|zoomed] [--width SIZE] [--height SIZE] [--workspace ID] [--target-pane PANE] [--direction right|down] [--cwd PATH] [--env KEY=VALUE] [--focus|--no-focus]");
    eprintln!("  gowild plugin pane focus <pane_id>");
    eprintln!("  gowild plugin pane close <pane_id>");
}
...
mod tests {
    ...
    fn github_plugin(
        ...
        InstalledPluginInfo {
            ...
            source: PluginSourceInfo {
                ...
                managed_path: Some(format!("/tmp/gowild/plugins/{id}")),
    ...
    fn github_plugin_source_parses_root_repo() {
        let source = GithubPluginSource::parse("ogulcancelik/gowild-plugin-examples").unwrap();
        assert_eq!(source.owner, "ogulcancelik");
        assert_eq!(source.repo, "gowild-plugin-examples");
        assert_eq!(source.subdir, None);
        assert_eq!(
            source.remote_url(),
            "https://github.com/ogulcancelik/gowild-plugin-examples.git"
        );
    }
    ...
    fn github_plugin_source_parses_subdir() {
        let source =
            GithubPluginSource::parse("ogulcancelik/gowild-plugin-examples/worktree-bootstrap")
                .unwrap();
        assert_eq!(source.owner, "ogulcancelik");
        assert_eq!(source.repo, "gowild-plugin-examples");
    ...
    fn github_plugin_source_rejects_non_shorthand_sources() {
        for source in [
            "https://github.com/ogulcancelik/gowild-plugin-examples",
            "git@github.com:ogulcancelik/gowild-plugin-examples.git",
            "ogulcancelik",
            "ogulcancelik/gowild-plugin-examples/../bad",
            "ogulcancelik/gowild-plugin-examples//bad",
        ] {
            assert!(
    ...
    fn github_source_lookup_matches_installed_plugin_source() {
        let source =
            GithubPluginSource::parse("ogulcancelik/gowild-plugin-examples/agent-telegram-notify")
                .unwrap();
        let plugins = vec![
            github_plugin(
                "examples.github-link-preview",
                "ogulcancelik",
                "gowild-plugin-examples",
