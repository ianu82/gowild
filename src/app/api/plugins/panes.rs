...
impl App {
    ...
    fn plugin_pane_launch_env(
        ...
        env.push(("GOWILD_ENV".to_string(), "1".to_string()));
        env.push(("GOWILD_PLUGIN_ID".to_string(), plugin.plugin_id.clone()));
        env.push((
            "GOWILD_PLUGIN_ENTRYPOINT_ID".to_string(),
            entrypoint.to_string(),
        ));
        env.push(("GOWILD_PLUGIN_CONTEXT_JSON".to_string(), context_json));
...
fn plugin_pane_protected_env_key(key: &str) -> bool {
    matches!(
        ...
        crate::api::SOCKET_PATH_ENV_VAR
            ...
            | "GOWILD_PLUGIN_ID"
            | "GOWILD_PLUGIN_ROOT"
            | "GOWILD_PLUGIN_CONFIG_DIR"
            | "GOWILD_PLUGIN_STATE_DIR"
            | "GOWILD_PLUGIN_ENTRYPOINT_ID"
            | "GOWILD_PLUGIN_CONTEXT_JSON"
            | "GOWILD_BIN_PATH"
    )
}
