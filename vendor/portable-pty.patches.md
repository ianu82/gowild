...
equivalent fix or exposes an option that lets GoWild keep the same behavior.
...
search path. GoWild must never load another application's DLL from `PATH`. The
Windows package ships a pinned Microsoft ConPTY runtime under `conpty/`; GoWild
...
`GOWILD_WINDOWS_CONPTY=system` to bypass the bundle during compatibility
recovery.

remove when: upstream `portable-pty` exposes hash-verified app-local and system
ConPTY selection with constrained DLL loading and no bare DLL search, or GoWild
...
reason: GoWild needs to launch `cmd.exe /d /c` with the user-authored command
...
support or GoWild replaces this launch path.
