---
name: gowild
description: "Control GoWild, a terminal multiplexer for coding agents. Use only when the user explicitly mentions GoWild or asks to use GoWild to inspect or control panes, tabs, workspaces, commands, or another agent. Do not use merely because a task could benefit from a background terminal, delegation, or parallel work. Requires GOWILD_ENV=1."
---

# GoWild

GoWild organizes terminals into workspaces, tabs, and panes, recognizes coding agents running inside panes, and exposes the current session through the `gowild` CLI.

Before issuing any control command, verify that this agent is running inside a GoWild-managed pane:

```bash
test "${GOWILD_ENV:-}" = 1
```

If the check fails, say that you are not running inside GoWild and stop. Do not inspect or control the focused GoWild session from outside GoWild.

When the check passes, the `gowild` binary in `PATH` talks to the current session. Use it to inspect neighboring work, create terminal layout, start agents and commands, read output, and wait for state changes.
...
Do not run bare `gowild` for discovery; it launches or attaches the TUI. Do not probe a mutating nested command by omitting arguments. Commands such as `gowild workspace create` are valid with defaults and will execute.
...
A pane exists whether or not it contains an agent. `agent start` requires an existing available shell pane and never creates, splits, or moves layout. Use pane commands for ordinary processes. Use agent commands when GoWild must validate agent identity or interpret `idle`, `working`, `blocked`, `done`, and `unknown` lifecycle states.
...
`idle` means the agent is ready for input and its tab has been seen in the focused GoWild UI. `done` is the same underlying idle state after unseen background work finishes. Focusing the tab or targeting the pane or agent with a focus command marks it seen. CLI reads do not mark it seen. `blocked` means GoWild recognized an approval or question UI. `unknown` means an agent is present but GoWild cannot classify it confidently; it does not prove completion.
...
GoWild injects the caller's context into each managed pane:

```bash
printf '%s\n' "$GOWILD_WORKSPACE_ID" "$GOWILD_TAB_ID" "$GOWILD_PANE_ID"
```
...
```bash
gowild workspace list
gowild tab list --workspace "$GOWILD_WORKSPACE_ID"
gowild pane current --current
gowild pane list --workspace "$GOWILD_WORKSPACE_ID"
gowild agent list
```
...
```bash
gowild pane layout --pane "$GOWILD_PANE_ID"
```
...
```bash
gowild pane split --current --direction right --cwd "$PWD" --no-focus
```
...
```bash
gowild agent start reviewer --kind codex --pane <returned-pane-id>
```

Use the kind requested by the user. Run `gowild agent` to inspect the installed kind list and options. Pass native agent arguments only after `--`:

```bash
gowild agent start reviewer --kind codex --pane <returned-pane-id> -- <agent-args...>
```

A successful `agent start` returns only after GoWild detects the expected agent in the same pane and considers it ready for interactive input. If the agent is blocked during startup, the command returns `agent_not_ready` immediately but keeps the name available for `agent read` and `agent send-keys`. Wait until the agent becomes idle before prompting it. Startup defaults to a 30-second timeout.

Submit work through the agent surface:

```bash
gowild agent prompt reviewer "Review the current diff and report only actionable findings." --wait --timeout 120000
```
...
```bash
gowild agent send-keys reviewer esc
gowild agent send-keys reviewer ctrl+c
```

GoWild validates all keys before writing any bytes. Read the result through the resolved agent:

```bash
gowild agent get reviewer
gowild agent read reviewer --source recent-unwrapped --lines 120
```
...
```bash
gowild pane run <returned-pane-id> "just test"
gowild pane wait-output <returned-pane-id> --match "test result" --timeout 120000
gowild pane read <returned-pane-id> --source recent-unwrapped --lines 120
```
...
`--lines` asks GoWild for more rows from the pane's available screen and host scrollback. If increasing it does not reveal more of a completed response, the pane is probably running the agent on the terminal's alternate screen. Rows that leave the alternate screen do not enter GoWild's host scrollback, so a larger line count cannot recover them.
...
- Never run `gowild server stop` from an active session unless the user explicitly intends to stop the server and its pane processes.
- Never kill the main GoWild process. Use named test sessions for experiments that need an isolated server.
