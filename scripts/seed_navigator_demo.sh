...
Seeds a running gowild server with navigator demo workspaces, tabs, panes,
and fake agent states for recording the session navigator.

Environment:
  GOWILD_NAV_SOCKET_PATH  API socket to target. Defaults to $HOME/.config/gowild-dev/gowild.sock.
  GOWILD_NAV_CWD          Workspace cwd for created panes. Defaults to the repo root.
  GOWILD_NAV_BIN          GoWild binary to call. Defaults to cargo run from the repo.
USAGE
}
...
workspace_cwd="${GOWILD_NAV_CWD:-$repo_dir}"
config_home="${XDG_CONFIG_HOME:-$HOME/.config}"
dev_socket="$config_home/gowild-dev/gowild.sock"
main_socket="$config_home/gowild/gowild.sock"
export GOWILD_SOCKET_PATH="${GOWILD_NAV_SOCKET_PATH:-$dev_socket}"

if [[ "$allow_main" != 1 && "$GOWILD_SOCKET_PATH" == "$main_socket" ]]; then
  echo "refusing to seed main gowild session: $GOWILD_SOCKET_PATH" >&2
  echo "use GOWILD_NAV_SOCKET_PATH for a dev socket, or pass --allow-main intentionally" >&2
  exit 1
fi

if [[ ! -S "$GOWILD_SOCKET_PATH" ]]; then
  echo "gowild socket not found: $GOWILD_SOCKET_PATH" >&2
  echo "start a dev server first, or set GOWILD_NAV_SOCKET_PATH" >&2
  exit 1
fi

cd "$repo_dir"

if [[ -n "${GOWILD_NAV_BIN:-}" ]]; then
  run() { "$GOWILD_NAV_BIN" "$@"; }
else
  run() { cargo run --quiet -- "$@"; }
fi
...
cat <<EOF
Seeded navigator demo data via $GOWILD_SOCKET_PATH
