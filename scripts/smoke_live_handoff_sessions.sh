...
GOWILD_BIN="${GOWILD_BIN:-$ROOT/target/debug/gowild}"
BASE="${BASE:-$(mktemp -d /tmp/gowild-handoff-smoke.XXXXXX)}"
...
cleanup() {
  ...
  for session in "${sessions[@]}"; do
    run_gowild "$session" server stop >/dev/null 2>&1 || true
  done
...
run_gowild() {
  ...
  env -u GOWILD_SOCKET_PATH \
    -u GOWILD_CLIENT_SOCKET_PATH \
    -u GOWILD_SESSION \
    XDG_CONFIG_HOME="$CONFIG_HOME" \
    XDG_RUNTIME_DIR="$RUNTIME_DIR" \
    XDG_STATE_HOME="$STATE_DIR" \
    "$GOWILD_BIN" --session "$session" "$@"
}

session_dir() {
  local session="$1"
  if [[ "$session" == "default" ]]; then
    printf '%s/gowild-dev' "$CONFIG_HOME"
  else
    printf '%s/gowild-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}

api_socket() {
  printf '%s/gowild.sock' "$(session_dir "$1")"
}

client_socket() {
  printf '%s/gowild-client.sock' "$(session_dir "$1")"
}

assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/gowild-dev/gowild.sock | "$CONFIG_HOME"/gowild-dev/sessions/*/gowild.sock)
      ;;
    *)
...
echo "using gowild: $GOWILD_BIN"
echo "smoke base: $BASE"

cargo build --locked --manifest-path "$ROOT/Cargo.toml" >/dev/null
mkdir -p "$CONFIG_HOME/gowild-dev" "$RUNTIME_DIR" "$STATE_DIR"
printf 'onboarding = false\n' > "$CONFIG_HOME/gowild-dev/config.toml"

for session in "${sessions[@]}"; do
  echo "starting smoke session $session at $(api_socket "$session")"
  run_gowild "$session" server >/dev/null 2>&1 &
  server_pids+=("$!")
