...
use support::{
    cleanup_test_base, register_runtime_dir, register_spawned_gowild_pid,
    unregister_spawned_gowild_pid,
};
...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/gowild-autodetect-test-{}-{nanos}",
        std::process::id()
    ))
}
...
/// Spawn `gowild` (no subcommand) — the auto-detect launch path.
fn spawn_gowild_auto(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
    _client_socket_path: &Path,
...
/// Spawn `gowild --no-session` — the monolithic escape hatch.
fn spawn_gowild_no_session(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
) -> SpawnedGoWild {
...
fn run_cli(socket_path: &Path, args: &[&str]) -> std::process::Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_gowild"));
    command.args(args);
    command.env("GOWILD_SOCKET_PATH", socket_path);
...
/// Running `gowild` with no server present starts a server
...
fn auto_detect_no_server_spawns_server_and_attaches() {
    ...
    // Run `gowild` (no subcommand) — should auto-detect, spawn server, attach as client.
    let gowild = spawn_gowild_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    let client_pid = gowild.child.process_id().expect("client should have PID");
    assert!(
        process_exists(client_pid),
        "client process should be running"
    );

    cleanup_spawned_gowild(gowild, base);
}

/// Running `gowild` with a server already running attaches
...
fn auto_detect_server_running_attaches_directly() {
    ...
    // Run `gowild` (no subcommand) — should detect the running server and attach.
    let client = spawn_gowild_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    cleanup_spawned_gowild(client, PathBuf::from("/nonexistent"));
    cleanup_spawned_gowild(server, base);
}

/// Socket path resolution is consistent between server and client.
/// Both derive the client socket from the `GOWILD_SOCKET_PATH` override,
...
fn auto_detect_socket_path_consistency() {
    ...
    // Run `gowild` with custom socket paths.
    let gowild = spawn_gowild_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
...
/// `gowild --no-session` bypasses server/client and runs
...
fn no_session_flag_runs_monolithically() {
    ...
    // Run `gowild --no-session` — monolithic mode, no server/client.
    let gowild = spawn_gowild_no_session(&config_home, &runtime_dir, &api_socket);
    ...
    // there is only one gowild process.
    let client_pid = gowild.child.process_id().expect("should have PID");
    assert!(
        process_exists(client_pid),
...
fn cli_subcommands_work_through_server() {
    ...
    // Test `gowild workspace list` through the server's API socket.
    ...
    // Test `gowild pane list` through the server's API socket.
...
/// persists after the client exits, and a new `gowild` can reattach.
#[test]
fn auto_detect_server_persists_and_reattaches() {
    ...
    // Run `gowild` — auto-detect spawns server + attaches client.
    let mut client1 = spawn_gowild_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    // Run `gowild` again — should detect the running server and reattach.
    let client2 = spawn_gowild_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    thread::sleep(Duration::from_millis(500));
    ...
    cleanup_spawned_gowild(client2, PathBuf::from("/nonexistent"));
    cleanup_test_base(&base);
}
...
fn auto_detect_default_socket_path_from_config_dir() {
    ...
    // Don't set GOWILD_SOCKET_PATH or GOWILD_CLIENT_SOCKET_PATH.
    ...
    let api_socket = config_home.join(app_dir_name).join("gowild.sock");
    let client_socket = config_home.join(app_dir_name).join("gowild-client.sock");
...
fn auto_detect_writes_client_and_server_logs_to_separate_files() {
    ...
    let spawned = spawn_gowild_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    let client_log = log_dir.join("gowild-client.log");
    let server_log = log_dir.join("gowild-server.log");
    let monolith_log = log_dir.join("gowild.log");
    ...
    assert!(
        ...
        "persistent client logs should not land in gowild.log: {monolith_content}"
    );
...
fn no_session_writes_startup_logs_to_monolith_file() {
    ...
    let spawned = spawn_gowild_no_session(&config_home, &runtime_dir, &api_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
...
fn auto_detect_respects_nested_guard_before_auto_attach() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_gowild"))
    ...
    assert!(
        stderr.contains("nested gowild is disabled by default"),
