...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/gowild-cross-area-test-{}-{nanos}",
        std::process::id()
    ))
}
...
fn spawn_server(config_home: &Path, runtime_dir: &Path, api_socket_path: &Path) -> SpawnedGoWild {
...
fn cross_area_detach_and_reattach_preserves_state() {
    ...
    // Use gowild: create a workspace and write output into its pane.
...
fn cross_area_agent_process_survives_detach_and_reattach() {
    ...
    let saw_working_on_client =
        wait_for_frame_matching(&mut client_b, Duration::from_secs(5), |frame| {
            frame_contains_colored_symbol(frame, "●", (251, 191, 36))
        })
    ...
    let saw_blocked_on_client =
        wait_for_frame_matching(&mut client_b, Duration::from_secs(5), |frame| {
            frame_contains_colored_symbol(frame, "●", (248, 113, 113))
        })
        .expect("frame decoding should succeed");
    assert!(
...
fn cross_area_server_kill_then_restart_and_reconnect() {
    ...
    cleanup_spawned_gowild(server2, base);
}
