...
fn named_sessions_use_separate_servers_and_workspace_state() {
    ...
    assert!(alpha_session["socket_path"]
        .as_str()
        .unwrap()
        .ends_with("/sessions/alpha/gowild.sock"));
...
fn dead_server_cli_reports_one_session_aware_json_line() {
    fn assert_server_not_running(
        ...
        assert_eq!(
            ...
            format!(
                "no gowild server is running at {}; run `{attach_command}` to start or attach it",
                socket_path.display()
            )
        );
    }
    ...
    assert_server_not_running(missing, &named_socket, "gowild session attach foo");

    let stale_socket = runtime_dir.join("stale.sock");
    drop(UnixListener::bind(&stale_socket).unwrap());
    let stale = Command::new(env!("CARGO_BIN_EXE_gowild"))
        .args(["workspace", "create"])
        .env("XDG_CONFIG_HOME", &config_home)
        .env("XDG_RUNTIME_DIR", &runtime_dir)
        .env("GOWILD_SOCKET_PATH", &stale_socket)
        .env("GOWILD_SESSION", "unrelated")
        .env_remove("GOWILD_CLIENT_SOCKET_PATH")
        .env_remove("GOWILD_ENV")
        .output()
        .unwrap();
    assert_server_not_running(stale, &stale_socket, "gowild");
...
fn integration_commands_run_locally_when_server_is_missing() {
    ...
    let expected_extension = extensions_dir.join("gowild-agent-state.ts");
    assert!(
        !expected_extension.exists(),
        "test setup should start without extension file"
    );

    let workspace_list = Command::new(env!("CARGO_BIN_EXE_gowild"))
        .args(["workspace", "list"])
        .env("GOWILD_SOCKET_PATH", &missing_socket)
        .env("HOME", &home_dir)
        .output()
        .unwrap();
    assert_eq!(workspace_list.status.code(), Some(1));

    let integration_install = Command::new(env!("CARGO_BIN_EXE_gowild"))
    ...
    let integration_status = Command::new(env!("CARGO_BIN_EXE_gowild"))
    ...
    let integration_uninstall = Command::new(env!("CARGO_BIN_EXE_gowild"))
...
fn integration_status_outdated_only_prints_action_for_legacy_install() {
    ...
    fs::create_dir_all(&extensions_dir).unwrap();
    fs::write(
        extensions_dir.join("gowild-agent-state.ts"),
        "// legacy gowild integration\n",
    )
    .unwrap();
    ...
    assert!(stderr.contains("installed gowild integrations need updating"));
    assert!(stderr.contains("gowild integration install pi"));
...
fn status_commands_report_client_and_server_versions() {
    ...
    let gowild = spawn_gowild(&config_home, &runtime_dir, &socket_path);
...
fn server_stop_command_shuts_down_running_server() {
    ...
    let mut gowild = spawn_gowild(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    ...
    let pid = gowild.child.process_id();
    let exit_status = gowild.child.wait().unwrap();
...
fn server_stop_then_restart_restores_pane_history() {
    ...
    let mut gowild = spawn_gowild_with_pane_history(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    wait_for_socket(&client_socket, Duration::from_secs(5));
    ...
    let restarted = spawn_gowild_with_pane_history(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    wait_for_socket(&client_socket, Duration::from_secs(5));
    ...
    cleanup_spawned_gowild(restarted, base);
}
