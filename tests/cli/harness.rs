...
pub(super) use crate::support::{
    ...
    unregister_spawned_gowild_pid, CURRENT_PROTOCOL,
};
...
pub(super) struct SpawnedGoWild {
...
pub(super) fn cleanup_spawned_gowild(spawned: SpawnedGoWild, base: PathBuf) {
    drop(spawned);
...
pub(super) fn spawn_gowild(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
) -> SpawnedGoWild {
    spawn_gowild_with_config(
        config_home,
        runtime_dir,
        socket_path,
        None,
...
pub(super) fn spawn_gowild_with_pane_history(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
) -> SpawnedGoWild {
...
pub(super) fn spawn_named_server(
    ...
    register_spawned_gowild_pid(Some(child.id()));
...
pub(super) fn run_named_cli_with_env_and_socket_override(
    ...
    if let Some(socket_override) = socket_override {
        command.env("GOWILD_SOCKET_PATH", socket_override);
    } else {
        command.env_remove("GOWILD_SOCKET_PATH");
    }
...
pub(super) fn spawn_gowild_with_path(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
...
pub(super) fn spawn_gowild_with_config(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
    path_override: Option<&Path>,
...
pub(super) fn parse_cli_json_output(
    ...
    serde_json::from_slice(&output.stdout).unwrap_or_else(|err| {
        panic!(
            "failed to parse JSON response for `gowild {}`: {}\nstdout: {}\nstderr: {}",
            args.join(" "),
            err,
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
